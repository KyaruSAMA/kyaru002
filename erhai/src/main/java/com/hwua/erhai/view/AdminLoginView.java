...
public class AdminLoginView extends Client {
    private User user = null;// 记录登录后的用户信息
    ...
    public void start() {
        ...
        // 创建请求对象
        LoginRequest loginRequest = new LoginRequest(Constant.LOGIN, userName, password, 1);
        // 将请求对象转化为JSON字符串
        ...
        if (loginResponse == null || loginResponse.getCode() == 400) { // 失败
            ...
            new AdminCarView(user).showCar(new String[]{"5"});//默认显示全部汽车
        }
    }
}
